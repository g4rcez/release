...
async function main() {
  const cli = new Cli("release", "v0.0.0", "ReleaseCLI: release process simplified!")
    ...
    .command("deploy", releaseCommand, { description: "Generate changelog file and publish release" })
    .command("changelog", releaseCommand, { description: "Generate changelog file and publish release (alias for deploy)" })
